...
public interface KdlDonHang extends JpaRepository<DonHang, Integer> {
    ...
    @Query("SELECT SUM(CAST(dh.tongTien AS BigDecimal)) FROM DonHang dh WHERE MONTH(dh.ngayTao) = :month AND YEAR(dh.ngayTao) = :year AND dh.trangThai = 'Da_GIAO'")
    BigDecimal getTotalRevenueByMonth(@Param("month") int month, @Param("year") int year);

    // Thống kê tổng doanh thu của năm
    @Query("SELECT SUM(CAST(dh.tongTien AS BigDecimal)) FROM DonHang dh WHERE YEAR(dh.ngayTao) = :year AND dh.trangThai = 'Da_GIAO'")
    ...
    List<DonHang> findByKhachHangId(int maKH);
}